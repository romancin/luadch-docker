pipeline {
  ...
  stages {
    stage('Cloning Git Repository') {
      steps {
        git url: 'https://github.com/romancin/luadch-docker.git',
            branch: '$BRANCH_NAME'
      }
    }
 ...
 post {
        success {
            telegramSend(message: '[Jenkins] - Pipeline CI-luadch-docker $BUILD_URL finalizado con estado :: $BUILD_STATUS', chatId: -395961814)
        }
    }
}
